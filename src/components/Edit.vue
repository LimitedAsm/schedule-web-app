<template>
  ...
  <div v-else>
    ...
    <div class="groups">
      <template v-for="group in this.groups" :key="group">
        <Group data-test="group" :group="group"></Group>
      </template>
    </div>
  </div>
</template>
...
export default {
  ...
  props: ["editDate"],
  emits: ["backToTimetable", "backToAuthentication"],
  data() {
    return {
      childrenFunction: [],
      date: this.editDate,
    ...
    async saveSchedule(isFinal) {
      ...
      this.childrenFunction.forEach((element) => {
        ...
        if (lessonInfo.delited == false) {
          ...
          let subjectSubgroup = this.getSubjectRefKey()(
            lessonInfo.subjectSubgroup
          );
          let employeeSubgroup = this.getEmployeeRefKey()(
            lessonInfo.employeeSubgroup
          );
