<template>
  ...
  <div class="main__info" data-test="lesson">
    ...
    <div class="select__menu">
      <div class="subgroup main__group">
        <input list="subject" placeholder="Предмет" v-model="lesson.subject" />
        <datalist id="subject" title="Предмет">
          <option v-for="subject in this.subjects" :key="subject">
            {{ subject }}
          </option>
        </datalist>
        <input
        ...
        <datalist id="employee" title="Преподаватель">
          <option v-for="employee in this.employees" :key="employee">
            {{ employee }}
          </option>
        </datalist>
        <input list="room" placeholder="Аудитория" v-model="lesson.room" />
        <datalist id="room" title="Аудитория">
          <option v-for="room in this.rooms" :key="room">
            {{ room }}
          </option>
        </datalist>
        <input 
  ...
  <button type="button" class="option__btn del" v-on:click="handleInsertLesson">
    <img src="../assets/paste.svg" />
  </button>

  <button type="button" class="option__btn del" v-on:click="handleCopyLesson">
  ...
  <button type="button" class="option__btn" v-on:click="shiftLessonDown">
    <img src="../assets/arrow-down.svg" />
  </button>

  <button type="button" class="option__btn" v-on:click="handleCleaningLesson">
...
export default {
  name: "Lesson",
  props: ["lessonNumber", "group"],
  emits: ["shiftLessonUp", "shiftLessonDown", "cleaningLesson"],
  inject: ["getLessonFunction", "getChildrenFunction"],
  data() {
    return {
      lesson: {
        room: "",
  ...
  created() {
    this.getChildrenFunction(
      this.returnChildrenFunctionGet,
      this.returnChildrenFunctionSet
    ),
      this.getLessonFunction(
        this.returnLessonFunctionGet,
        this.returnLessonFunctionSet,
        this.lessonNumber
      );
  },
  mounted() {
    ...
    if (this.getSchedule() != "noSchedule") {
      this.getSchedule().forEach((schedule) => {
        let lesson = this.lesson;
        if (
          schedule.groupKey == this.getGroupRefKey()(this.group) &&
          schedule.lessonNumber == this.lessonNumber
        ) {
          if (schedule.subgroup == 0) {
            ...
            lesson.employeeSubgroup = this.getEmployeeByRefKey()(
              schedule.teacherKey
            );
            lesson.subjectSubgroup = this.getSubjectByRefKey()(
              schedule.subjectKey
            );
          }
        }
